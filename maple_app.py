...
# --- טופס הזנה חכם: חישוב זמנים אוטומטי ---
    ...
    # --- טופס הזנה חכם: גרסת כפתורי פלוס-מינוס ---
    with st.form("smart_train_form", clear_on_submit=True):
        now = datetime.now(IL_TZ)
        st.write("**מתי האימון התחיל?**")
        
        # שורת בחירת שעת התחלה עם כפתורי פלוס מינוס
        c1, c2, c3 = st.columns([1, 1, 2])
        
        with c1:
            # ברירת מחדל: השעה של לפני שעה
            default_hour = (now - timedelta(hours=1)).hour
            start_h = st.number_input("שעה", min_value=0, max_value=23, value=default_hour, step=1)
        
        with c2:
            default_minute = now.minute
            # כפתורי פלוס מינוס שקופצים ב-5 דקות לכל לחיצה - נוח מאוד!
            start_m = st.number_input("דקות", min_value=0, max_value=59, value=default_minute, step=5)
        ...
        # שעת הסיום נשארת "עכשיו" כברירת מחדל, אבל עם רכיב נוח
        st.write("**מתי הוא הסתיים?**")
        end_time = st.time_input("שעת סיום (ברירת מחדל: עכשיו)", value=now.time())
        
        # -- חישוב הזמן --
        dt_start = datetime.combine(d_date, time(int(start_h), int(start_m)))
        dt_end = datetime.combine(d_date, end_time)
        
        # טיפול במקרה שהאימון עבר את חצות
        if dt_end < dt_start:
        ...
        if calc_duration > 0:
            st.info(f"⏳ משך מחושב: **{calc_duration:.2f} שעות**")
        else:
            st.warning("שים לב: שעת הסיום לפני שעת ההתחלה")

        # --- שאר השדות ---
        d_stress = st.select_slider("מדד לחץ", options=[1, 2, 3, 4, 5], value=3)
        d_note = st.text_area("הערות")
        
        if st.form_submit_button("שמור אימון 💾", use_container_width=True):
            # שמירה בפורמט HH:MM נקי
            formatted_start = f"{int(start_h):02d}:{int(start_m):02d}"
            row = [str(d_date), formatted_start, round(calc_duration, 2), d_stress, d_note]
